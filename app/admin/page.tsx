...
export default function Page() {
  ...
  const startCrawler = async (test?: boolean) => {
    try {
      ...
      const crawlerUrl = test
        ? `${baseUrl}/api/v1/test-crawler/simple`
        : `${baseUrl}/api/v1/crawler/start`;
  ...
  return (
    <div className="flex flex-col items-center space-y-4 p-6">
      ...
      {message && (
        <div
          className={`mt-4 p-4 rounded-lg border ${status === 'success' ? 'bg-green-50 border-green-200 text-green-800' : status === 'error' ? 'bg-red-50 border-red-200 text-red-800' : 'bg-blue-50 border-blue-200 text-blue-800'}`}
        >
      ...
      <div className="mt-6 text-xs text-muted-foreground max-w-md text-center">
        <p>
          ⚠️ 크롤링은 백그라운드에서 실행되며, 완료까지 시간이 걸릴 수 있습니다. 페이지를
          새로고침하지 마세요.
        </p>
      </div>
    </div>
