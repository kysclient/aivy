...
import { CreateMealPlanDto, MealPlan, MealPlanRepository, MealPlanStatus } from '@/repositoires/MealPlanRepository';
...
const mutators = {
    ...
    updateMealPlan: async (url: string, { arg }: { arg: { id: string; data: Partial<CreateMealPlanDto> } }) => {
...
export function useGeneratingMealPlans() {
    const { mealPlans, total, page, limit, totalPages, hasNext, hasPrev, isLoading, error, refresh } = useMealPlansByStatus(MealPlanStatus.GENERATING);
    return {
