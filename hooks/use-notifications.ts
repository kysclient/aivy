...
export function useNotifications(
  ...
  } = useSWR(NOTIFICATION_KEYS.all(page, limit, unreadOnly), fetchers.getNotifications, {
...
export function useNotificationPreferences() {
  ...
  } = useSWR(NOTIFICATION_KEYS.preferences(), fetchers.getPreferences, {
    revalidateOnFocus: false,
    revalidateOnReconnect: true,
...
export function useUnreadCount() {
  const {
    ...
    isLoading,
    mutate: refreshCount,
  } = useSWR(NOTIFICATION_KEYS.unreadCount(), fetchers.getUnreadCount, {
...
export function useMarkNotificationAsRead() {
  const { trigger, isMutating } = useSWRMutation('mark-as-read', mutators.markAsRead);
...
export function useMarkAllAsRead() {
  const { trigger, isMutating } = useSWRMutation('mark-all-as-read', mutators.markAllAsRead);
...
export function useUpdateNotificationPreferences() {
  const { trigger, isMutating } = useSWRMutation('update-preferences', mutators.updatePreferences);
...
export function useNotificationManager() {
  ...
  const refreshAllCaches = async () => {
    await Promise.all([mutate((key) => Array.isArray(key) && key[0] === 'notifications')]);
  };
