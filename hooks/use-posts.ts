import {
  ...
  PostType,
  UpdatePostDto,
} from '@/repositoires/PostRepository';
...
const POST_KEYS = {
  ...
  userPosts: (page?: number, limit?: number) => ['posts', 'user', { page, limit }],
...
const fetchers = {
  ...
  getAllPosts: async ([, , params]: [
    string,
    string,
    { page?: number; limit?: number; postType?: PostType },
  ]) => {
...
export function useCreatePost() {
  const { trigger, isMutating } = useSWRMutation('create-post', mutators.createPost);
...
export function useUpdatePost() {
  const { trigger, isMutating } = useSWRMutation('update-post', mutators.updatePost);
...
export function useDeletePost() {
  const { trigger, isMutating } = useSWRMutation('delete-post', mutators.deletePost);
...
export function useToggleLike() {
  const { trigger, isMutating } = useSWRMutation('toggle-like', mutators.toggleLike);
