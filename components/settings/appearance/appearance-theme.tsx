...
export function AppearanceTheme() {
  ...
  const baseButtonClass = useMemo(
    () =>
      'cursor-pointer font-semibold flex-1 p-3 text-sm flex justify-center bg-background border-r border-border text-description last:border-r-0 transition-colors duration-200',
    []
  );
      ...
      <div className="rounded-lg overflow-hidden w-full flex border border-border">
        {THEME_OPTIONS.map((option) => (
          <button
            ...
            className={cn(
              baseButtonClass,
              isActiveTheme(option.value) && 'bg-foreground text-background'
            )}
          ...
          <div className="rounded-lg overflow-hidden w-full flex border border-border">
            {DARK_THEME_OPTIONS.map((option) => (
              <button
                ...
                className={cn(
                  baseButtonClass,
                  isActiveDarkTheme(option.value) && 'bg-foreground text-background'
                )}
                aria-label={`${option.label} 선택`}
                aria-pressed={isActiveDarkTheme(option.value)}
              >
                {option.label}
              </button>
            ))}
