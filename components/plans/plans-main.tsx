...
export function PlansMain() {
  const { mealPlans, total, page, totalPages, isLoading, error, refresh: mutate } = useUserMealPlans()
  ...
  return (
    <>
      {testDatas.map((mealPlan: MealPlan) => (
        <div
          key={mealPlan.id}
          // onClick={() => onPlanClick?.(mealPlan)}
          className={cn('group relative overflow-hidden border border-border/50', 'bg-card/50 backdrop-blur-sm', 'hover:bg-card hover:border-border transition-all duration-200', 'hover:shadow-lg hover:shadow-primary/5', 'cursor-pointer')}
        >
          ...
          <div className="relative flex items-center gap-4 p-4 relative">
            ...
            <div className="flex-shrink-0">
              <div className={cn('inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium border', getStatusColor(mealPlan.status))}>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex flex-col gap-1 sm:gap-2">
                  <div className="min-w-0 flex-1">
                    <h3 className="font-semibold text-foreground text-lg leading-tight mb-2 group-hover:text-primary transition-colors">{mealPlan.title}</h3>

                    {mealPlan.dailyCalories && <p className="text-sm text-muted-foreground mt-2 line-clamp-1">평균칼로리: {mealPlan.dailyCalories}kcal</p>}
                  </div>
