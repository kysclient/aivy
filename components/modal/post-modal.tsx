        ...
        <DialogContent showCloseButton={false} className="w-full p-0 py-2.5 max-w-lg">
          ...
          <div className="flex gap-4 px-2.5 min-h-[180px]">
            <div className="relative w-12 h-12 rounded-full flex-shrink-0 overflow-hidden">
              <Image src={user?.profileImage || `https://avatar.vercel.sh/${user?.name || 'user'}`} alt={user?.nickname || '사용자 아바타'} width={48} height={48} className="object-cover" />
            </div>
          ...
          <div className="border-t border-border px-2.5 pt-2.5">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                ...
                <svg width="24" height="24" viewBox="0 0 36 36" className="flex-shrink-0" aria-hidden="true">
                  <circle cx="18" cy="18" r="16" fill="none" strokeWidth="2" className="stroke-muted-foreground" />
                  <circle cx="18" cy="18" r="16" fill="none" stroke="#FF3263" strokeWidth="4" strokeDasharray="100" strokeDashoffset={100 - progress} transform="rotate(-90 18 18)" className="transition-all duration-300 ease-in-out" />
                </svg>
              </div>
            </div>
          </div>
