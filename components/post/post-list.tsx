...
function PurePostList({ postType }: PostListProps) {
  ...
  const { posts, hasNext, isLoading, error, refresh } = usePosts(page, limit, postType);
            ...
            <>
              ...
              >
                {isLoading || isLoadingMore ? (
                  ...
                  <div className="text-sm text-muted-foreground opacity-50">No more posts</div>
                )}
              </div>
              ...
              {!hasNext && accumulatedPosts.length > 0 && (
                <div className="py-4 flex justify-center items-center">
                  <div className="text-sm text-muted-foreground  pt-4">마지막 게시물입니다</div>
                </div>
              )}
            </>
          ) : (
