...
function PurePostItem({
  ...
  return (
    <div
      className={cn(
        ' relative',
        !isOnPost && 'hover:bg-muted',
        !isChild && 'border-t border-border'
      )}
      onClick={(e) => !isOnPost && postBodyOnClick(e, post.id)}
    >
      ...
      <div className={cn('flex flex-row px-[15px] pt-[12px]', isOnPost && 'flex-row')}>
        <div className="relative flex flex-col items-center w-[50px] flex-shrink-0">
          ...
          {isParent && (
            <div className="absolute top-[42px] left-[26px] w-[2px] h-[calc(100%-6.5px)] bg-border" />
          )}
        </div>
          ...
          {post.image && (
            ...
            >
              {Array.isArray(post.image) ? (
                <ImageGallery images={post.image} onModalOpenChange={setOpenImageModal} />
              ) : (
                <ImageGallery images={[post.image]} onModalOpenChange={setOpenImageModal} />
              )}
            </div>
